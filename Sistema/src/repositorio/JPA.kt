...
)
{
    ...
    // \d+ -> um ou mais digitos [0-9] | \. -> ponto obrigatorio | \d{2} -> 2 digitos obrigatorios
    val regexValor = Regex("^\\d+\\.\\d{2}$")

    fun regex(texto: String, regex: Regex, erro: String) : String {
        while (true){
            println(texto)
            val entrada = readlnOrNull()?.trim().orEmpty()
