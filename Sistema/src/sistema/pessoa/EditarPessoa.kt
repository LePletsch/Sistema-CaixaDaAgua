...
fun EditarPessoa(){
    ...
    val tipo = conexao.tipoPessoa(id)
    ...
    val pessoa: Pessoa = when (tipo) {
        "CLIENTE" -> {
            ...
            Cliente(cpf = cpf, nome = nome, email = email, telefone = telefone, tipo = Pessoas.CLIENTE)
        }
        "FUNCIONARIO" -> {
            val cpf = jpa.regex("Digite o cpf: ", conexao.regexCPF, "CPF inválido. Use o formato 000.000.000-00 ou 11 dígitos.")
            var setor = 0
            ...
            Funcionario(cpf = cpf, setor = Setor.entries[setor], salario = salario.toBigDecimal(), turno = Turno.entries[turno], nome = nome, email = email, telefone = telefone, tipo = Pessoas.FUNCIONARIO)
        }
        "FORNECEDOR" -> {
            val cnpj = jpa.regex("Digite o cnpj: ", conexao.regexCNPJ, "CNPJ inválido. Use o formato 00.000.000/0000-00 ou 14 dígitos.")
            Fornecedor(cnpj = cnpj, nome = nome, email = email, telefone = telefone, tipo = Pessoas.FORNECEDOR)
        }
        else -> {
    ...
    try {
        conexao.editarPessoas(pessoa,id)
